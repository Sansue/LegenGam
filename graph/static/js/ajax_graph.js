...
function handle_sorting(i, order) {
    ...
    let sortedIntList = combinedList.map(obj => obj.int);
...
function add_new_viewer(i) { // THE ULTIMATE FUNCTION TO RULE THEM ALL
    ...
    fetch(request)
        .then(response => response.json())
        .then(result => {
            viewer.innerHTML = result['html']; //putting all the html in the newly created component
            create_viewer_objects(i, formData['html_type']);
        })
        .catch(error =>{
...
function create_viewer_objects(i, html_type){ // handling the creation of the default graph, and all the variables and buttons needed for the viewer
    ...
    if(html_type === 'base') {
        ...
        let default_chart = {
            ...
            Graph: new Chart(context, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Example Chart, choose a dataset to change',
                ...
                options: {scales: {yAxes: [{ticks: {min: 40, max: 85}}]}, legend: {position: "bottom"}}
            }),
        ...
        downloadPdf.addEventListener("click", function () {
            ...
            pdf.setFontSize(40);
            pdf.text(10, reportPageHeight / 1.78 + 30, "Dataset : " + all_charts[i].current_dataset, {
                horizontalScale: 2,
                charSpace: 5
            }); //dataset
